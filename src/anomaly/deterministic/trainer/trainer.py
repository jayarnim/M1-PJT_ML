...
class Trainer(object):
    ...
    def progressor(self, trn_loader, val_loader):
        ...
        for epoch in range(self.num_epochs):
            ...
            trn_log, val_log = self.engine(**kwargs)

            kwargs = dict(
                current_score=val_log["loss"],
                current_state=copy.deepcopy(self.model.state_dict()),
                current_anomaly=val_log["anomaly"],
            )
            self.monitor(**kwargs)

            # ACCUMULATE ==========
            trn_log_list.append(trn_log["loss"])
            val_log_list.append(val_log["loss"])
            ...
            # PRINT ==========
            print(
                f'CURRENT TRN LOSS: {trn_log["loss"]:.4f}',
            )
            print(
                f'CURRENT VAL LOSS: {val_log["loss"]:.4f}',
        ...
        return dict(
            trn=trn_log_list,
            val=val_log_list,
            anomaly=self.monitor.best_anomaly,
        )
